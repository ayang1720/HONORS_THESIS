...
axs.set_title('October 1st at 6-UTC',fontsize=24)
#scatter plot
lonCell=np.rad2deg(lonCell)
latCell=np.rad2deg(latCell)
cs=axs.scatter(lonCell,latCell,c=height_500hPa,cmap='jet',s=50,marker='*',transform=ccrs.PlateCarree())
cbar=plt.colorbar(cs,orientation='horizontal')
cbar.set_label('500 mb Geometric Heights (in meters)',fontsize=24)

#tricontourf plots (i have never used this module before, so i got help from chatGPT)

plt.savefig('plot_scatter_500mb_heights.png')
plt.close()